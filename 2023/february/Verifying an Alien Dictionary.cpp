            ...
            for (int i = 0; i < words.size() - 1; i++) {
                ...
                for (int j = 0; j < words[i].length(); j++) {
                    ...
                    if (words[i][j] != words[i + 1][j]) {
                        int currentWordChar = words[i][j];
                        int nextWordChar = words[i + 1][j];
                        if (positionMap[currentWordChar] > positionMap[nextWordChar]) return false;
...
void testVerifyingAlienDictionary() {
    // Input: words = ["hello","leetcode"], order = "hlabcdefgijkmnopqrstuvwxyz"
    // Output: true
    ...
    cout << "Verifying an Alien Dictionary: " << Solution().isAlienSorted(words, order) << endl;
}
