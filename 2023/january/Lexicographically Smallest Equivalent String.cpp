...
namespace {
    ...
    public:
        string smallestEquivalentString(string A, string B, string S) {
            vector<int> ds(26, -1);
            for (int i = 0; i < A.size(); ++i) ds_merge(ds, A[i] - 'a', B[i] - 'a');
            for (int i = 0; i < S.size(); ++i) S[i] = ds_find(ds, S[i] - 'a') + 'a';
            return S;
        }
...
void testLexicographicallySmallestEquivalentString() {
    // Input: s1 = "parker", s2 = "morris", baseStr = "parser", Output: "makkek"
    string s1 = "parker", s2 = "morris", baseStr = "parser";
    cout << "LexicographicallySmallestEquivalentString: " << Solution().smallestEquivalentString(s1, s2, baseStr) << endl;
}
