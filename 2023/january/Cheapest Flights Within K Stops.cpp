...
namespace {
    ...
    public:
        int findCheapestPrice(int n,
            ...
            for (int i = 0; i <= k; ++i) {
                ...
                for (auto flight : flights) {
                    ...
                    if (previous[u] < INT_MAX) {
                        current[v] = min(current[v], w + previous[u]);
                    }
                }
            ...
            return current[dst] != INT_MAX ? current[dst] : -1;
        }
    };
}  // namespace

void testCheapestFlightsWithKStops() {
    /*
        Input: n = 4, flights = [[0,1,100],[1,2,100],[2,0,100],[1,3,600],[2,3,200]], src = 0, dst = 3, k = 1
        Output: 700
    */
    ...
    cout << "Cheapest Flights Within K Stops: " << Solution().findCheapestPrice(n, flights, src, dst, k) << endl;
