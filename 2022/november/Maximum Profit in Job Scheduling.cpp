...
namespace {
    ...
    public:
        int jobScheduling(
            ...
            vector<vector<int>> jobs; // {end, start, profit}
            for (int i = 0; i < n; ++i) {
                jobs.push_back({
                    endTime[i], // end
                    startTime[i], // start
...
void testMaximumProfitInJobScheduling() {
    /*
        Input: startTime = [1,2,3,3], endTime = [3,4,5,6], profit = [50,10,40,70]
        Output: 120
    */
    ...
    cout << "Maximum Profit in Job Scheduling: " 
        << Solution().jobScheduling(startTime, endTime, profits)
        << endl;
}
